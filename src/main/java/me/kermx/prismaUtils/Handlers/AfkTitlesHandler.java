...
public class AfkTitlesHandler extends BukkitRunnable {
    ...
    private boolean isPlayerAfk(Player player){
        String afkStatus = PlaceholderAPI.setPlaceholders(player, ConfigUtils.getInstance().afkPlaceholder);
        return afkStatus.equalsIgnoreCase("true");
    }

    private String getRandomTitle(){
        return ConfigUtils.getInstance().afkTitles.get((int) (Math.random() * ConfigUtils.getInstance().afkTitles.size()));
    }

    private String getRandomSubtitle(){
        return ConfigUtils.getInstance().afkSubtitles.get((int) (Math.random() * ConfigUtils.getInstance().afkSubtitles.size()));
    }
